...
class CocoDatasetGenerator:
    ...
    def load_gt_dataset(self, image_path, ann_file):
        ...
        print('loaded {} records from coco dataset.'.format(cnt), flush=True)
    ...
    def load_detect_dataset(self, image_path, ann_file, bbox_file):
        ...
        print('Total boxes: {}'.format(len(all_boxes)), flush=True)
    ...
    def __getitem__(self, idx):
        ...
        if data_numpy is None:
            # print('[ERROR] fail to read {}'.format(image_file), flush=True)
...
def create_dataset_coco(
        rank=0, group_size=1, train_mode=True, num_parallel_workers=8, transform=None, shuffle=None):
    """
    CreateDatasetCoco
    """
    ...
    if train_mode:
        image_path = os.path.join(image_path, config["DATASET"]["TRAIN_SET"])
        ann_file = os.path.join(ann_file, config["DATASET"]["TRAIN_JSON"])
    else:
        image_path = os.path.join(image_path, config["DATASET"]["TEST_SET"])
        ann_file = os.path.join(ann_file, config["DATASET"]["TEST_JSON"])
    bbox_file = os.path.join(bbox_file, config["TEST"]["COCO_BBOX_FILE"])

    print('loading dataset from {}'.format(image_path), flush=True)

...
def evaluate(cfg, preds, output_dir, all_boxes, img_id, ann_path):
    ...
    try:
        from pycocotools.coco import COCO
        from pycocotools.cocoeval import COCOeval
        print("================ load pycocotools ================", flush=True)
        print("================ load pycocotools ================", flush=True)
        has_coco = True
    except ImportError:
        has_coco = False
    print("================================", flush=True)
    print("================================", flush=True)
    print("==========coco: {}==============".format(has_coco), flush=True)
    print("==========coco: {}==============".format(has_coco), flush=True)
    print("================================", flush=True)
    print("================================", flush=True)
    ...
    if 'test' not in image_set and has_coco:
        ann_path = ann_path if ann_path else os.path.join(
            cfg["DATASET"]["ROOT"], cfg["DATASET"]["ANNOTATIONS_SET"], "person_keypoints_val2017.json")
